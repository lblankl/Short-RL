...
def CosFn(t,T,min,max):
    res=min+(max-min)*(1+math.cos(t*math.pi/T))/2
    return res
class LengthRScorer:
    def __init__(self,data,tokenizer,config):
        ...
        for i in self.id2len.keys():
            if len(self.id2len[i]) > 0:
                self.id2mean[i] = sum(self.id2len[i])/len(self.id2len[i])
                self.id2std[i] = math.sqrt(sum([(x - self.id2mean[i]) ** 2 for x in self.id2len[i]]) / len(self.id2len[i]))
            else:
                self.id2mean[i] = 0
                self.id2std[i] = 0
        global MaxACC
    ...
    def compute_score(self,
        ...
        if answer_score > 0:
            len_mean = self.id2mean[self.index[index]]
            len_std = self.id2std[self.index[index]]
            print("len_mean",len_mean)
            print("len_std",len_std)
            #sigmoid((current_len-len_mean)/len_std)
            fn= sigmoid((current_len-len_mean)/(len_std+0.001))
            print("fn :", fn)
            total_score=total_score*(1-alpha*fn)
            
            print("correct scores:",total_score)

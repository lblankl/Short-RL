model="/volume/ailab4sci/txie/ydl/short_ablation2/ShortRL-logic1e-6-200-1/actor/global_step_1686" #model path
config="vllm"
num_limit=100
max_token=8192
ntrain=0
...
#     --problem_type "clean" --eval_nppl ${eval_nppl} --log_path $log_path/{$eval_nppl}.json > "$log_file" 2>&1 &
# done & 

# wait

python avg.py --log_path $log_path
...
class KimiScorer:
    def __init__(self,data,config):
        ...
        for i in range(bsz):
            ...
            self.id2len[index[i]].append(valid_response_length)
            if index[i] not in self.id2maxlen:
                self.id2maxlen[index[i]] = valid_response_length
                self.id2minlen[index[i]] = valid_response_length
            else:
                if valid_response_length > self.id2maxlen[index[i]]:
                    self.id2maxlen[index[i]] = valid_response_length
                if valid_response_length < self.id2minlen[index[i]]:
                    self.id2minlen[index[i]] = valid_response_length
    ...
    def compute_score(self,
        ...
        len_reward = length_penalty if answer_score > 0 else min(0, length_penalty)

        

        total_score = format_score + answer_score + len_reward*self.config.algorithm.alpha
        # print("\n" + "-"*80)

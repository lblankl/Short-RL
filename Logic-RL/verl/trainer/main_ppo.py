...
import torch
from verl.utils.reward_score import gsm8k, math, multiply, countdown, kk, kk_kimi, kk_lengthr, kk_lengthrG
...
def _select_rm_score_fn(data_source,config,data,tokenizer):
    ...
    elif config.trainer.reward_type == 'kk_kimi':
        return kk_kimi.KimiScorer(data)
    elif config.trainer.reward_type == 'kk_lengthr':
        return kk_lengthr.LengthRScorer(data,tokenizer)
    elif config.trainer.reward_type == 'kk_lengthrG':
        return kk_lengthrG.LengthRScorer(data,tokenizer,config)
    else:
        raise NotImplementedError

def _select_val_rm_score_fn(data_source,config,data):
...
class ValRewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            compute_score_fn = _select_val_rm_score_fn(data_source,self.config,data)
...
def main_task(config):
    ...
    val_reward_fn = ValRewardManager(tokenizer=tokenizer, num_examine=1,config=config )
